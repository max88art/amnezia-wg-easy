...
module.exports = class WireGuard {
  ...
  async __buildConfig() {
    this.__configPromise = Promise.resolve().then(async () => {
      ...
      let config;
      try {
        config = await fs.readFile(path.join(WG_PATH, 'wg8.json'), 'utf8');
  ...
  async getConfig() {
    if (!this.__configPromise) {
      ...
      await Util.exec('wg-quick down wg8').catch(() => {});
      await Util.exec('wg-quick up wg8').catch((err) => {
        if (err && err.message && err.message.includes('Cannot find device "wg8"')) {
          throw new Error('WireGuard exited with the error: Cannot find device "wg8"\nThis usually means that your host\'s kernel does not support WireGuard!');
        }
      ...
      // await Util.exec('iptables -A FORWARD -i wg8 -j ACCEPT');
      // await Util.exec('iptables -A FORWARD -o wg8 -j ACCEPT');
...
${client.preSharedKey ? `PresharedKey = ${client.preSharedKey}\n` : ''
    ...
    debug('Config saving...');
    await fs.writeFile(path.join(WG_PATH, 'wg8.json'), JSON.stringify(config, false, 2), {
      mode: 0o660,
    });
    await fs.writeFile(path.join(WG_PATH, 'wg8.conf'), result, {
      mode: 0o600,
    });
    debug('Config saved.');
  }
  ...
  async __syncConfig() {
    ...
    await Util.exec('wg syncconf wg8 <(wg-quick strip wg8)');
    debug('Config synced.');
  }
  ...
  async getClients() {
    ...
    const dump = await Util.exec('wg show wg8 dump', {
      log: false,
    });
    dump
      .trim()
...
Endpoint = ${WG_HOST}:${WG_CONFIG_PORT}`;
  ...
  async getMetrics() {
    ...
    for (const client of Object.values(clients)) {
      ...
      wireguardSentBytes += `wireguard_sent_bytes{interface="wg8",enabled="${client.enabled}",address="${client.address}",name="${client.name}"} ${Number(client.transferTx)}\n`;
      wireguardReceivedBytes += `wireguard_received_bytes{interface="wg8",enabled="${client.enabled}",address="${client.address}",name="${client.name}"} ${Number(client.transferRx)}\n`;
      wireguardLatestHandshakeSeconds += `wireguard_latest_handshake_seconds{interface="wg8",enabled="${client.enabled}",address="${client.address}",name="${client.name}"} ${client.latestHandshakeAt ? (new Date().getTime() - new Date(client.latestHandshakeAt).getTime()) / 1000 : 0}\n`;
    }
    ...
    returnText += `wireguard_configured_peers{interface="wg8"} ${Number(wireguardPeerCount)}\n`;

    returnText += '\n# HELP wireguard_enabled_peers\n';
    returnText += '# TYPE wireguard_enabled_peers gauge\n';
    returnText += `wireguard_enabled_peers{interface="wg8"} ${Number(wireguardEnabledPeersCount)}\n`;

    returnText += '\n# HELP wireguard_connected_peers\n';
    returnText += '# TYPE wireguard_connected_peers gauge\n';
    returnText += `wireguard_connected_peers{interface="wg8"} ${Number(wireguardConnectedPeersCount)}\n`;
